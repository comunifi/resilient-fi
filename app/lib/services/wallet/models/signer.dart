...
class Signature extends MsgSignature {
  ...
  factory Signature.fromMsgSignature(MsgSignature msgSignature) {
    return Signature(msgSignature.r, msgSignature.s, msgSignature.v);
  }
  ...
  Map<String, dynamic> toJson() {
    return {'r': r.toString(), 's': s.toString(), 'v': v};
  }
}
...
bool verifySignature(SignatureVerifier verifier) {
  try {
    final recoveredAddress = bytesToHex(
      publicKeyToAddress(verifier.publicKey),
      include0x: true,
    );
