...
import 'package:app/services/indexer/signed_request.dart';
import 'package:app/services/wallet/contracts/erc20.dart';
import 'package:app/services/wallet/contracts/profile.dart';
import 'package:app/services/wallet/models/json_rpc.dart';
import 'package:app/services/wallet/models/paymaster_data.dart';
...
Uint8List tokenTransferCallData(
  ...
  if (config.getPrimaryToken().standard == 'erc20') {
    ...
    return config.token1155Contract.transferCallData(
      from.hexEip55,
      to,
      tokenId ?? BigInt.zero,
      amount,
...
Future<String?> setProfile(
  ...
  try {
    ...
    final resp = await config.engineIPFSService.filePut(
      ...
      file: image,
      fileType: fileType,
      headers: {'X-Signature': sig, 'X-Address': account.hexEip55},
      body: body.toJson(),
    );

    final String profileUrl = resp['object']['ipfs_url'];

    final calldata = config.profileContract.setCallData(
      account.hexEip55,
      profile.username,
      profileUrl,
    );
...
Future<bool> accountExists(Config config, EthereumAddress account) async {
  try {
    final url = '/v1/accounts/${account.hexEip55}/exists';

    await config.engine.get(url: url);
...
Future<SUJSONRPCResponse> requestPaymaster(
  ...
  final rawResponse = await config.engineRPC.post(body: body);
